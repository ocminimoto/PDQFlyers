...
if (!isset($_REQUEST["buildURL"])) {
  ...
  if (isset($_REQUEST["MAXITEMS"])) {
    $FeedMaxItemsFeedMaxItems = $_REQUEST["MAXITEMS"];
  }
      ...
      if (!function_exists("FeedForALL_rss2html_replaceInItem")) {
        Function FeedForALL_rss2html_replaceInItem($source, $currentItem) {
		  ...
		  if($striptags) {
			  ...
			  //FeedForAll_rss2html_limitLength($currentItem->description, $limitItemDescriptionLength)
			  $descrInsert = strip_tags( $currentItem->description );			  
			  $item = FeedForAll_rss2html_str_replace("~~~ItemDescription~~~", FeedForAll_rss2html_limitLength($descrInsert, $limitItemDescriptionLength), $item);

		  }
		  else {
			  $item = FeedForAll_rss2html_str_replace("~~~ItemDescription~~~", FeedForAll_rss2html_limitLength($currentItem->description, 0), $item);
		  } 
